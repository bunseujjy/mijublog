...
import { getCurrentUserCmt, getRepliesResponse } from '~/server/comments/getResponse';
        ...
        <TabsContent value="response">
          <div class="py-6">
            <template v-if="isLoading">
              <div class="text-center py-4 text-black dark:text-white">Loading...</div>
            </template>
            
            <template v-else-if="error">
              <div class="text-center text-red-500 py-4">{{ error.message }}</div>
            </template>
            ...
            <template v-else-if="responses.length || replyResponse.length">
                ...
                <ResponseList :response="response" :reply="null" :author="findPostAuthor(response.user_id)"/>
            </li>
            <li 
                v-for="reply in replyResponse"
                :key="reply.id"
                class="border-b border-b-muted pb-5 transition-all duration-300 ease-in-out mt-5">
                <ResponseList :response="null" :reply="reply" :author="findPostAuthor(reply.user_id)" />
            </li>
            </template>
