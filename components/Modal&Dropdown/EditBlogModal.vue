...
const title = ref(props.title || props.blog.title)
const subtitle = ref(props.subtitle || props.blog.subtitle)
const tags = ref<string[]>(props.tags || props.blog.tags || [])
  ...
  try {
    ...
    for (const tag of addedTags) {
      ...
      const { data: existingCategory } = await client
        ...
        .single();
      if (existingCategory) {
        // Increment post_length for the existing category
        await client
          .from("categories")
          .update({ post_length: existingCategory.post_length! + 1 })
          .eq("id", existingCategory.id);
        // Set the category_id to the existing category's ID
        categories_id.value = existingCategory.id; 
      } else {
        ...
        const { data: newCategory } = await client
          ...
          .single();
        if (newCategory) {
          // Set the category_id to the new category's ID
    ...
    navigateTo('/', { external: true })
      ...
      toast({description: 'Blog must at least have one category.', variant: 'destructive'})
    }
  } catch (error) {
          ...
          <div class="flex flex-col md:flex-row items-center gap-6 py-4">
            <div class="w-full md:w-1/2 space-y-4">
              <div class="space-y-2">
                ...
                <div v-if="props.blog.featured_image_url" class="relative aspect-video overflow-hidden rounded-lg">
                  <NuxtImg format="webp" loading="lazy" :src="props.blog.featured_image_url" alt="Cover image" class="object-cover w-full h-full" />
                ...
                <div v-if="preview_img" class="relative aspect-video overflow-hidden rounded-lg">
                  <NuxtImg format="webp" loading="lazy" :src="preview_img" alt="Cover image" class="object-cover w-full h-full" />
                  <Button size="icon" variant="secondary" class="absolute right-2 top-2" @click="removepreview_img">
                    <X class="h-4 w-4" />
                  </Button>
                </div>
