...
<template>
  <div class="container mx-auto px-4 py-8 md:px-0 transition-colors duration-300" ref="containerRef" id="blog_post">
    ...
    <div class="mb-6 flex items-center flex-wrap gap-2">
      ...
      <button v-for="cat in categories.slice(0, 6)" :key="cat.id" @click="selectedCategory = cat.name" :class="[
      ...
      <div class="w-full lg:w-[30%] mt-10 lg:mt-0 lg:ml-3">
        <div class="mb-8">
          ...
          <div v-if="props.blog_db.length > 0">
            <div v-for="(blog, idx) in props.blog_db
              ?.slice(0, 4)
              .filter((data) => data.visibility !== 'private')" :key="blog.id" class="mb-4">
        ...
        <div class="mb-8">
          ...
          <div class="flex flex-wrap gap-2">
            <NuxtLink v-for="cat in categories.slice(0, 5)" :key="cat.id" :to="`/categories/${cat.slug}`"
        ...
        <div class="mb-8">
          ...
          <div v-if="props.blog_db?.length > 0">
            <div v-for="(rec, idx) in props.blog_db
              ?.slice(0, 6)
              .filter((data) => data.visibility !== 'private')" :key="idx" class="flex items-start mb-4">
              ...
              <div>
                <NuxtLink :to="`/categories/${rec.tags[0].toLocaleLowerCase()}`">
                  <span :class="[
                    'text-white text-xs rounded-full px-2 py-[1px] mb-1 inline-block',
                    getTagColor(idx),
                  ]">{{ rec.tags[0] }}</span>
                </NuxtLink>
                <NuxtLink :to="`/post/@${getAuthorDetails(users, rec.author_id)?.user_metadata?.username}/${rec.id}`">
