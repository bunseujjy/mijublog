...
const emit = defineEmits(['reply', 'toggleAction', 'onClickOutsideHandler', 'handleDeletingReply']);
...
const handleLikeReply = async () => {
  await likeReplies(1, props.reply?.id ?? '', props.currentUser?.id as string);
};
...
<template>
  <div class="comment-thread relative" :class="{ 'ml-14': isSecondLevel }" :id="reply?.id" v-click-outside="onClickOutsideHandler" ref="ignoreElRef">
    <div :class="['flex', {'my-7' : hasChildReplies}]">
      <div class="mr-3 flex flex-col items-center relative">
        <NuxtImg format="webp" loading="lazy" :src="ownerOfReplies?.user_metadata?.profile_url" alt="profile" class="size-[40px] bg-blue-500 rounded-full object-cover" :placeholder="15"/>
      ...
      <div class="flex-grow">
          ...
          <div class="text-black dark:text-white mt-0.5 mb-3 flex space-x-4">
            ...
            <div class="relative" ref="moreRef">
              ...
              <div v-show="isOpenEdit[reply?.id ?? '']">
                <EditingCommentModal :isOpenEdit="isOpenEdit" @toggleAction="toggleAction" :comment="null" :reply="reply" :currentUser="currentUser" :blog_db="blog_db" :list_db="list_db"/>
              </div>
            </div>
          </div>
        ...
        <div v-if="isExpanded && hasChildReplies" class="mt-4 space-y-4">
          <div v-for="childReply in childReplies.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())" :key="childReply.id">
            <CommentLoading v-if="replyLoading" :comments="[]" :reply="childReplies"/>
            <NestedComment
              v-else 
              :comment="comment" 
              :reply="childReply" 
